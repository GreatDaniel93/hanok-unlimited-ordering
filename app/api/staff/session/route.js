...
export async function POST(request) {
  ...
  try {
    const { data, error } = await db().rpc('staff_start_session', {
      p_secret: backendSecret(), p_actor: role, p_table_id: String(body.table_id || ''),
      p_adults: clampInt(body.adults,0,30), p_children_8_12: clampInt(body.children_8_12,0,30),
      p_children_4_7: clampInt(body.children_4_7,0,30), p_under_4: clampInt(body.under_4,0,30)
    });
...
export async function PATCH(request) {
  ...
  const payload = action==='extend' ? { minutes: clampInt(body.minutes,1,30) }
    ...
    : {};
  try {
    const { data, error } = await db().rpc('staff_session_action', { p_secret: backendSecret(), p_actor: role, p_session_id: sessionId, p_action: action, p_payload: payload });
    if (error) return jsonError(error.message, 409);
    return Response.json(data);
  } catch (error) { return jsonError(error.message,503); }
}
