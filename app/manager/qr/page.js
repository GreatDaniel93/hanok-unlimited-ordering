...
function QRCard({table,origin}){
  ...
  function download(){
    ...
    a.href=src;
    a.download=`Hanok-Wagga-${table.name}-QR.png`;
    document.body.appendChild(a);a.click();a.remove();
  }
  return <div className="card qr-card" style={{textAlign:'center',breakInside:'avoid'}}>
    <div style={{fontSize:26,fontWeight:900,letterSpacing:1}}>{table.name}</div>
    <div className="muted" style={{fontSize:12,marginTop:2}}>HANOK WAGGA WAGGA · TABLE ORDERING</div>
    <div style={{margin:'14px auto',width:'100%',maxWidth:260,aspectRatio:'1/1',display:'grid',placeItems:'center',background:'#fff',borderRadius:12,padding:8}}>{src?<img src={src} alt={`${table.name} QR code`} style={{width:'100%',height:'100%',objectFit:'contain'}}/>:<span className="muted">{error||'Generating…'}</span>}</div>
    <div style={{fontSize:12,fontWeight:700}}>SCAN TO ORDER</div>
    <div className="muted qr-url" style={{fontSize:9,wordBreak:'break-all',marginTop:6}}>{url}</div>
    <button className="btn secondary small no-print" style={{marginTop:12}} disabled={!src} onClick={download}>Download PNG</button>
  </div>;
}
...
export default function ManagerQRPage(){
  ...
  async function load(){
    ...
    if(r.status===401){setAuth(false);setTables([]);return;}
    if(!r.ok){setError(j.error||'Unable to load tables.');return;}
    if(j.role!=='manager'){
      await fetch('/api/auth/logout',{method:'POST'});
      setAuth(false);setTables([]);setError('Manager PIN required.');return;
    }
