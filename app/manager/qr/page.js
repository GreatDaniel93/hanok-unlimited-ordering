...
function QRCard({table}){
  ...
  return <div className="label-wrap"><div className="qr-label"><div className="qr-box">{src?<img src={src} alt={`${table.name} QR code`}/>:<span>{error||'Generating…'}</span>}</div><div className="table-number">{table.name}</div></div></div>;
}
...
export default function ManagerQRPage(){
  ...
  return <><style>{`
    .label-sheet{display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:22px;margin-top:18px}.label-wrap{display:flex;justify-content:center}.qr-label{width:200px;height:200px;box-sizing:border-box;border-radius:50%;background:#050505;border:3px solid #050505;box-shadow:inset 0 0 0 2px #e31b23,inset 0 0 0 7px #050505;padding:22px 24px 14px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;overflow:hidden}.qr-box{width:120px;height:120px;background:#fff;border-radius:8px;padding:5px;box-sizing:border-box;display:grid;place-items:center}.qr-box img{width:100%;height:100%;display:block}.qr-box span{color:#555;font-size:7px}.table-number{color:#e31b23;font-size:32px;line-height:32px;font-weight:950;letter-spacing:-1px;margin-top:7px}
    @media print{@page{margin:5mm}.no-print,.topbar{display:none!important}.page{max-width:none!important;padding:0!important;margin:0!important}.label-sheet{display:grid!important;grid-template-columns:repeat(3,50mm)!important;grid-auto-rows:50mm!important;gap:3mm!important;justify-content:center!important;margin:0!important}.label-wrap{width:50mm!important;height:50mm!important;break-inside:avoid!important;page-break-inside:avoid!important}.qr-label{width:50mm!important;height:50mm!important;box-shadow:inset 0 0 0 .45mm #e31b23,inset 0 0 0 1.6mm #050505!important;padding:5.5mm 6mm 3.5mm!important}.qr-box{width:30mm!important;height:30mm!important;border-radius:2mm!important;padding:1.1mm!important}.table-number{font-size:8mm!important;line-height:8mm!important;margin-top:1.8mm!important}body{background:#fff!important}}
  `}</style><div className="topbar no-print"><div className="logo">HANOK<small>WAGGA WAGGA · TABLE QR CODES</small></div><div className="spacer"/><a href="/manager/tables" className="btn secondary small">Table Management</a><a href="/manager/menu" className="btn secondary small">Manager Menu</a><a href="/staff" className="btn secondary small">Staff Dashboard</a><button className="btn secondary small" onClick={logout}>Logout</button></div><main className="page" style={{maxWidth:1120}}><section className="hero no-print"><h1>50mm Round Table QR Codes</h1><p>Simple QR + table number layout for all active tables.</p><div className="notice" style={{marginTop:12}}><b>Official QR domain:</b> {PRODUCTION_ORIGIN}<br/><b>Print size:</b> 50mm × 50mm · Scale 100% / Actual Size · Disable “Fit to page”.</div><div className="actions" style={{marginTop:14}}><button className="btn brand" disabled={!tables.length} onClick={()=>window.print()}>PRINT ALL {tables.length} ACTIVE TABLES</button></div></section>{error&&<div className="error" style={{marginTop:12}}>{error}</div>}{!tables.length&&!error?<div className="card" style={{marginTop:16}}>No active tables found.</div>:<div className="label-sheet">{tables.map(t=><QRCard key={t.id} table={t}/>)}</div>}</main></>;
}
