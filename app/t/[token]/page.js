    ...
    <main className="page" style={{maxWidth:760}}>
      ...
      {data&&<>
        ...
        {!session ? <div className="notice" style={{marginTop:14}}><b>Your table is not active yet.</b><br/>Please wait for our team to start your dining session.</div> : <>
          {isLunch?<div className="notice" style={{marginTop:14}}><b>60-Minute Weekday Lunch Buffet</b><br/>Hot dishes, rice and soup can be ordered here. Side dishes and desserts are self-service. BBQ meats are not included. Last order closes with 10 minutes remaining.</div>:<div className="notice" style={{marginTop:14}}><b>Hanok First Grill Selection</b><br/>Your starter platter has been sent to the meat station. Side dishes and desserts are self-service.</div>}
          ...
          <div className="grid grid-2">
            {menu.map(item=>{const q=cart[item.id]||0;const wait=item.station==='meat'?meatWait:item.station==='hot'?hotWait:0;const itemMax=Number(item.max_per_round)||0;return <div className="card" key={item.id} style={{display:'grid',gridTemplateColumns:'1fr auto',gap:12,alignItems:'center'}}>
              <div><h3 style={{margin:'0 0 4px',fontSize:16}}>{item.display_name||item.name}</h3><div className="muted" style={{fontSize:12}}>{item.portion_label||item.description}</div>{itemMax>0&&<div style={{fontSize:11,fontWeight:700,marginTop:6}}>Max {itemMax} per order</div>}{wait>0&&<div style={{fontSize:11,color:'#8a5010',marginTop:6}}>Next order in {formatTime(wait)}</div>}</div>
              <div className="actions" style={{alignItems:'center',flexWrap:'nowrap'}}><button className="btn secondary small" disabled={submitting} onClick={()=>change(item,-1)}>−</button><b>{q}{itemMax>0?` / ${itemMax}`:''}</b><button className="btn secondary small" disabled={submitting||wait>0||lastOrderClosed||(itemMax>0&&q>=itemMax)||(item.category==='meat'&&(isLunch||meatCount>=meatLimit))} onClick={()=>change(item,1)}>+</button></div>
            </div>})}
          </div>
          <div className="card" style={{position:'sticky',bottom:12,marginTop:16,background:'#241c18',color:'#fff',zIndex:20}}>
            <div style={{display:'flex',alignItems:'center',gap:12}}><div><b>{itemCount} items</b>{!isLunch&&<div style={{fontSize:12,color:'#d8c7aa'}}>Meat {meatCount} / {meatLimit} this round</div>}{isLunch&&<div style={{fontSize:12,color:'#d8c7aa'}}>Lunch Buffet · no BBQ meats</div>}</div><div className="spacer"/><button className="btn gold" disabled={!itemCount||submitting||lastOrderClosed} onClick={submit}>{submitting?'SENDING…':'PLACE ORDER'}</button></div>
          </div>
