...
function lastOrderClosed(session){return session ? Date.now()>=new Date(session.last_order_at).getTime() : false;}
...
export default function StaffPage(){
  ...
  return <>
    ...
    <main className="page">
      <section className="hero"><h1>Table Control</h1><p>Start Weekday Lunch Buffet or Unlimited BBQ sessions and manage the dining flow.</p></section>
      ...
      {!data?<div className="card" style={{marginTop:16}}>Loading…</div>:<>
        ...
        <div className="grid grid-4">{data.tables.map(t=>{const s=t.session;const m=s?minsLeft(s.ends_at):null;const last=s&&lastOrderClosed(s);const bbqLabel=s?.starter_preference==='none'?'BBQ · NO STARTER':s?.starter_preference==='no_pork'?'BBQ · NO PORK STARTER':'BBQ · STANDARD STARTER';return <button key={t.id} onClick={()=>{setSelected(t.id);setError('')}} className="card" style={{textAlign:'left',cursor:'pointer',background:selected===t.id?'#f5e6dc':'var(--panel)'}}><div className="actions" style={{justifyContent:'space-between'}}><b style={{fontSize:22}}>{t.name}</b><span className={`badge ${!s?'available':last?'last':'dining'}`}>{!s?'AVAILABLE':last?'LAST ORDER':'DINING'}</span></div><div style={{marginTop:14,fontSize:13}}>{!s?<span className="muted">Ready to start</span>:<><b>{s.adults+s.children_8_12+s.children_4_7+s.under_4} guests</b><br/><span className="muted">{m} min remaining · {s.service_mode==='lunch'?'WEEKDAY LUNCH · 60 MIN':bbqLabel}</span></>}</div></button>})}</div>
      </>}
      {selectedTable&&<div className="card" style={{marginTop:18}}>
        ...
        {!selectedTable.session?<>
          ...
          <div className="field" style={{maxWidth:520,marginBottom:14}}><label>Service Type</label><select value={form.service_mode} onChange={e=>setForm(f=>({...f,service_mode:e.target.value}))}><option value="lunch">Weekday Lunch Buffet · 60 min</option><option value="bbq">Unlimited BBQ · 90 min</option></select></div>
          ...
          {form.service_mode==='bbq'&&<div className="field" style={{marginTop:14,maxWidth:420}}><label>Starter Preference</label><select value={form.starter_preference} onChange={e=>setForm(f=>({...f,starter_preference:e.target.value}))}><option value="standard">Standard Starter</option><option value="no_pork">No Pork Starter</option><option value="none">No Starter · Guests choose their own meat</option></select></div>}
          {form.service_mode==='lunch'?<div className="notice" style={{margin:'14px 0'}}><b>WEEKDAY LUNCH BUFFET</b><br/>60-minute dining session · Last order closes with 15 minutes remaining · No BBQ meat · No Starter Platter. Customers can order all active Hot Dishes and Rice & Soup items.</div>:form.starter_preference==='none'?<div className="notice" style={{margin:'14px 0'}}><b>NO STARTER</b><br/>No Starter Platter will be sent. Guests can immediately choose their own BBQ meats from the QR menu, subject to the normal meat-per-round limit and cooldown after each meat order.</div>:<div className="notice" style={{margin:'14px 0'}}>Starting the table will begin the 90-minute timer and automatically send the correct <b>{form.starter_preference==='no_pork'?'NO PORK':'STANDARD'}</b> Starter Platter to the Meat Station.</div>}
          <button className="btn brand" disabled={busy} onClick={start}>{form.service_mode==='lunch'?'START 60-MIN LUNCH':'START 90-MIN BBQ SESSION'}</button>
          {form.service_mode==='lunch'&&data.role!=='manager'&&<p className="muted" style={{fontSize:12}}>Weekday Lunch Buffet is available Monday to Friday. Managers can open a test session outside weekdays.</p>}
        </>:<>
          <div className="notice" style={{margin:'14px 0'}}><b>{selectedTable.session.service_mode==='lunch'?'WEEKDAY LUNCH BUFFET':'UNLIMITED BBQ'}</b><br/><b>{minsLeft(selectedTable.session.ends_at)} min remaining</b><br/>Guests: {selectedTable.session.adults} adults · {selectedTable.session.children_8_12} age 8–12 · {selectedTable.session.children_4_7} age 4–7 · {selectedTable.session.under_4} under 4<br/>{selectedTable.session.service_mode==='lunch'?<span>Last order closes 15 minutes before session end · BBQ meat is disabled.</span>:selectedTable.session.starter_preference==='none'?<><b>Starter:</b> NONE · Guests choose their own meat</>:<><b>Starter:</b> {selectedTable.session.starter_preference==='no_pork'?'NO PORK':'STANDARD'}</>}</div>
