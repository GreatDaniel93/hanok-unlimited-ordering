  ...
  return <>
    ...
    <main className="page">
      ...
      {!data?<div className="card" style={{marginTop:16}}>Loading…</div>:<>
        ...
        <div className="grid grid-4">{data.tables.map(t=>{const s=t.session;const m=s?minsLeft(s.ends_at):null;const last=s&&lastOrderClosed(s);return <button key={t.id} onClick={()=>{setSelected(t.id);setError('')}} className="card" style={{textAlign:'left',cursor:'pointer',background:selected===t.id?'#f5e6dc':'var(--panel)'}}><div className="actions" style={{justifyContent:'space-between'}}><b style={{fontSize:22}}>{t.name}</b><span className={`badge ${!s?'available':last?'last':'dining'}`}>{!s?'AVAILABLE':last?'LAST ORDER':'DINING'}</span></div><div style={{marginTop:14,fontSize:13}}>{!s?<span className="muted">Ready to start</span>:<><b>{s.adults+s.children_8_12+s.children_4_7+s.under_4} guests</b><br/><span className="muted">{m} min remaining · {s.service_mode==='lunch'?'WEEKDAY LUNCH · 60 MIN':s.starter_preference==='no_pork'?'BBQ · NO PORK STARTER':'BBQ · STANDARD STARTER'}</span></>}</div></button>})}</div>
      </>}
      {selectedTable&&<div className="card" style={{marginTop:18}}>
        ...
        {!selectedTable.session?<>
          ...
          {form.service_mode==='bbq'&&<div className="field" style={{marginTop:14,maxWidth:360}}><label>Starter Preference</label><select value={form.starter_preference} onChange={e=>setForm(f=>({...f,starter_preference:e.target.value}))}><option value="standard">Standard</option><option value="no_pork">No Pork</option></select></div>}
          {form.service_mode==='lunch'?<div className="notice" style={{margin:'14px 0'}}><b>WEEKDAY LUNCH BUFFET</b><br/>60-minute dining session · Last order closes with 10 minutes remaining · No BBQ meat · No Starter Platter. Customers can order all active Hot Dishes and Rice & Soup items.</div>:<div className="notice" style={{margin:'14px 0'}}>Starting the table will begin the 90-minute timer and automatically send the correct <b>{form.starter_preference==='no_pork'?'NO PORK':'STANDARD'}</b> Starter Platter to the Meat Station.</div>}
