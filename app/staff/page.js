...
export default function StaffPage(){
  const [data,setData]=useState(null);const [auth,setAuth]=useState(true);const [error,setError]=useState('');const [selected,setSelected]=useState(null);const [form,setForm]=useState({adults:2,children_8_12:0,children_4_7:0,under_4:0});const [busy,setBusy]=useState(false);const [,tick]=useState(0);
  ...
  return <>
    ...
    <main className="page">
      ...
      {!data?<div className="card" style={{marginTop:16}}>Loading…</div>:<>
        ...
        <div className="grid grid-4">{data.tables.map(t=>{const s=t.session;const m=s?minsLeft(s.ends_at):null;return <button key={t.id} onClick={()=>{setSelected(t.id);setError('')}} className="card" style={{textAlign:'left',cursor:'pointer',background:selected===t.id?'#f5e6dc':'var(--panel)'}}><div className="actions" style={{justifyContent:'space-between'}}><b style={{fontSize:22}}>{t.name}</b><span className={`badge ${!s?'available':m<=15?'last':'dining'}`}>{!s?'AVAILABLE':m<=15?'LAST ORDER':'DINING'}</span></div><div style={{marginTop:14,fontSize:13}}>{!s?<span className="muted">Ready to start</span>:<><b>{s.adults+s.children_8_12+s.children_4_7+s.under_4} guests</b><br/><span className="muted">{m} min remaining · Round {s.round_count}</span></>}</div></button>})}</div>
      </>}
      {selectedTable&&<div className="card" style={{marginTop:18}}>
        ...
        {!selectedTable.session?<>
          <div className="section-title"><h3>Start New Session</h3></div><div className="grid grid-4">{[['adults','Adults'],['children_8_12','Children 8–12'],['children_4_7','Children 4–7'],['under_4','Under 4']].map(([k,l])=><div className="field" key={k}><label>{l}</label><input type="number" min="0" value={form[k]} onChange={e=>setForm(f=>({...f,[k]:Number(e.target.value)}))}/></div>)}</div><div className="notice" style={{margin:'14px 0'}}>Starting the table will begin the 90-minute timer and automatically send the correct Starter Platter to the Meat Station.</div><button className="btn brand" disabled={busy} onClick={start}>START 90-MIN SESSION</button>
        </>:<>
          <div className="notice" style={{margin:'14px 0'}}><b>{minsLeft(selectedTable.session.ends_at)} min remaining</b><br/>Guests: {selectedTable.session.adults} adults · {selectedTable.session.children_8_12} age 8–12 · {selectedTable.session.children_4_7} age 4–7 · {selectedTable.session.under_4} under 4</div>
          ...
          <div className="section-title"><h3>Customer QR URL</h3></div><code style={{display:'block',overflowX:'auto',padding:12,background:'#eee4d8',borderRadius:10}}>{`${typeof window!=='undefined'?window.location.origin:''}/t/${selectedTable.token}`}</code>
        </>}
      </div>}
    </main>
  </>;
